...
void print(const std::string& str, bool is_start = true) {
    std::cout << (is_start ? "Inizio " : "Fine ") << str << ": ";
    if (not is_start) std::cout << std::endl;
}
...
struct C1 {
  int i1;
  C1() {
    std::cout << "Constructor C1::C1()" << std::endl;
  }
  ~C1() {
...
struct C2 {
  int i2;
  C2() {
    std::cout << "Constructor C2::C2()" << std::endl;
    throw 123;
  }
  ~C2() {
...
struct C3 : public C1 {
  int i3;
  C3() {
    std::cout << "Constructor C3::C3()" << std::endl;
  }
  ~C3() {
...
public:
  D() : c1(), c2(), c3() {
    std::cout << "Constructor D::D()" << std::endl;
  }
  ~D() {
...
int main() {
  ...
  try {
      print("C1 riga 101");
      C1 c1;
      print("C1 riga 103", false);
      print("D riga 104");
      D d;
      print("D riga 106", false);
  }
  catch (char c) {
