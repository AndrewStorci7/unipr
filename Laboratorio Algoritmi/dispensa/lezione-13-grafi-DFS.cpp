...
int DFS(int n) {
    ...
    if (V_visitato[n]==1) // se gia' visitato --> ciclo!
        return 1;

    if (V_visitato[n]==2) // se gia' visitato --> altro percorso
        return 0;
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < n_nodi; i++) {
        ...
        for (int j = 0; j < n_nodi; j++) {
            // for (int j = n_nodi-1; j >= 0; j--) {// ordine inverso
            // if (rand() % 2 == 0)
            // if (i < j)   /// dag
            // if ((i + 1) % n_nodi == j)
            // if ((j + 1) / 2 == i + 1)    // albero
